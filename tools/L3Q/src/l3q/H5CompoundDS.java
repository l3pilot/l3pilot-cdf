...
public class H5CompoundDS extends CompoundDS {
        ...
        if (did >= 0) {
            ...
            try {
                ...
                for (int i = 0; i < atomicList.size(); i++) {
                    ...
                    try {
                        int factor = 1;
                        if (member_type.getDatatypeClass() == 10) factor = 4;
                     
                    	member_data = member_type.allocateArray((int) (totalSelectedSpacePoints*factor)); 
                        // Sami: more buffer space needed, since size calculation is wrong. Even *4, if all are bytes
                    }
                    ...
                    if (member_data != null) {
                        ...
                        try {
                        	...
                        	long atom_tid = member_type.createNative();
                        	  int member_class =  member_type.getDatatypeClass(); //H5.H5Tget_class(atom_tid); 
                              int compInfo[] = { member_class, member_size, 0 };
                        	comp_tid = createCompoundFieldType(atom_tid, flatNameList.get(i), compInfo);
                        }
                        catch (HDF5Exception ex) {
                            System.out.println("is array "+member_type.isArray());
                        ...
                        if (member_type.isUnsigned()) {
                            ...
                            if (getJavaObjectRuntimeClass(member_data) == 'B') {
                            ...
                            try {
                                ...
                                //Sami: this is now wrong, read bytes do not match member_size
                                ...
                                long memberLengths2[] = null;
                                long memberTypes[] = null;
                                int memberClasses[] = null;
                                int numberOfMembers;
                                ...
                                try {
                                    ...
                                    memberLengths2 = new long[numberOfMembers];
                                    // Sami: additional check for lengths
                                    ...
                                    for (int j = 0; j < numberOfMembers; j++) {
                                        ...
                                        memberLengths2[j] = H5.H5Tget_size(memberTypes[j]);
                                        memberClasses[j] = H5.H5Tget_class( memberTypes[j]);
                                    }
                                    ...
                                    for (int j = 0; j < numberOfMembers; j++) {
                                        if (j < numberOfMembers - 1) {
                                        	...
                                        	// Sami: this fix may go wrong, if last number length differs in byte buffer
                                            if (memberLengths2[j] == 1) memberLengths[j] = 4;
                                            else memberLengths[j] = memberLengths2[j]; //(compoundSize - memberOffsets[j]);
                                        }
                                        compoundSize += memberLengths[j];
                                    }
                                    
                                    for (int j = 0; j < numberOfCompounds; j++) {
                                    	//for (int j = 0; j < 1; j++) { testing one line
                                        ...
                                        for (int k = 0; k < numberOfMembers; k++) {
                                        	...
                                        	//            memberLengths[k], memberClasses[k]);

                                        	Object currentData = null;

                                        	try {
                                        		// Sami: this part of the code has been made shorter. Some format conversions are still likely missing.
                                        		byte[] partToConvert = partArray((byte[])member_data, (int) (memberOffsets[k] + (compoundSize * j)), (int) memberLengths[k]);
                                        		
                                        		if (memberClasses[k] == HDF5Constants.H5T_INTEGER) {

                                        			if(memberLengths2[k] == 2)
                                        			{
                                        				//currentData = HDFNativeData.byteToShort((byte[])member_data, (int) (memberOffsets[k] + (compoundSize * j)));
                                        				//Sami: byteToShort call does not work properly, converts inputted array in full and is slow
                                        				currentData = HDFNativeData.byteToShort(partToConvert)[0];
                                        			}
                                        			else if(memberLengths2[k] == 1)
                                        				currentData = partToConvert[0];
                                        				//currentData = HDFNativeData.byteToInt((int) (memberOffsets[k] + (compoundSize * j)), (int) memberLengths[k], (byte[])member_data);
                                        			else 
                                        				HDFNativeData.byteToInt(partToConvert);
                                        				//System.out.println("unsupported typeclass "+memberClasses[k]+" size "+memberLengths2[k]);
                                        		}
                                        		else if (memberClasses[k] == HDF5Constants.H5T_FLOAT) {
                                        			
                                        			if(memberLengths2[k] == 4)
                                        				currentData = HDFNativeData.byteToFloat(partToConvert)[0];
                                        			//currentData = HDFNativeData.byteToDouble((int) (memberOffsets[k] + (compoundSize * j)), (int) memberLengths[k], (byte[])member_data);
                                        			else
                                        				currentData = HDFNativeData.byteToDouble(partToConvert)[0];
                                        		}
                                        		else if (memberClasses[k] == HDF5Constants.H5T_ENUM) {
                                        			//System.out.println("unsupported enum typeclass");
                                        			currentData = partToConvert[0];
                                        		}
                                        		else System.out.println("unsupported typeclass "+memberClasses[k]);
                                        	}
                                        	catch (Exception ex) {
                                        		log.debug("convertCompoundByteMember(): conversion failure: ", ex);
                                        	}

                                        		((Object[]) field_data)[k] = currentData;
                                        	//	((Object[]) field_data)[k] = Array.get(currentData, 0);
                                        }
    ...
    private static byte[] partArray(byte[] array, int start, int size) {
     ...
     */
    private Object convertCompoundByteMember(byte[] data, long data_type, long start, long len, int typeClass) {
        Object currentData = null;
        ...
        try {
            ...
            if (typeClass == HDF5Constants.H5T_INTEGER) {
                ...
                currentData = HDFNativeData.byteToInt((int) start, (int) len, data);
            }
            else if (typeClass == HDF5Constants.H5T_FLOAT) {
            	//long size = H5.H5Tget_size(data_type);
                currentData = HDFNativeData.byteToDouble((int) start, (int) len, data);
            }
            else if (typeClass == HDF5Constants.H5T_ENUM) {
            	//System.out.println("unsupported enum typeclass");
            	currentData = HDFNativeData.byteToByte(data[(int)start]);
            }
            else System.out.println("unsupported typeclass "+typeClass);
        }
        catch (Exception ex) {
            log.debug("convertCompoundByteMember(): conversion failure: ", ex);
        }
    ...
    private final long createCompoundFieldType(long atom_tid, String member_name, int[] compInfo) throws HDF5Exception {
        // Sami: this version is from previous HDF5, but can't remember if that was needed, after all

                    ...
                    try {
                        ...
                        if (attr.getDatatype().isVarStr()) {
                            ...
                            try {
                                ...
                                // H5.H5AreadComplex(aid, tid, strs); // fails with strings, native lib contains some bug
                                long size = H5.H5Tget_size(tid);
                                byte[] buf = new byte[(int)size]; // size

                                //int tclass = H5.H5Tget_class(tid);
                                //if (tclass == HDF5Constants.H5T_COMPOUND) // probably unncessary check

                                H5.H5Aread(aid, tid, buf); // this reads it nicely but gives only pointers to strings
                                // currently there is no way to get the strings e.g. with H5AreadVL
                                // for implementation in C, search e.g. "h5str_sprintf"
                                value = buf;

                                // we get something about the strings anyhow
                                H5.H5AreadComplex(aid, tid, strs);
                                attr.dataStr = strs; // a new variable for L3Q use only
